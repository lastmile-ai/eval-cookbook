...
client = openai_tracing.wrap(OpenAI(max_retries=5), tracer)


@tracer.start_as_current_span("get_embedding")
def get_embedding(text: str, model="text-embedding-3-small", **kwargs) -> List[float]:
    tracer.register_param("raw_text", text)
    tracer.register_param("model", model)
   
    # replace newlines, which can negatively affect performance.
    text = text.replace("\n", " ")
    tracer.register_param("text_to_embed", text)
...
@tracer.start_as_current_span("aget_embedding")
...
def get_embeddings(
    ...
    tracer.register_param("raw_list_of_text", list_of_text)
    assert len(list_of_text) <= 2048, "The batch size should not be larger than 2048."

    # replace newlines, which can negatively affect performance.
    list_of_text = [text.replace("\n", " ") for text in list_of_text]
    tracer.register_param("list_to_text_to_embed", list_of_text)

    data = client.embeddings.create(input=list_of_text, model=model, **kwargs).data
    return [d.embedding for d in data]

@tracer.start_as_current_span("aget_embeddings")
...
@tracer.start_as_current_span("cosine_similarity")
...
@tracer.start_as_current_span("distances_from_embeddings")
def distances_from_embeddings(
    ...
    tracer.register_param("query_embedding", query_embedding)
    tracer.register_param("embeddings", embeddings)
    tracer.register_param("distance_metric", distance_metric)
...
@tracer.start_as_current_span("pca_components_from_embeddings")
...
@tracer.start_as_current_span("tsne_components_from_embeddings")
